...
struct MainHomeView: View {
    ...
    func makeHeader(headerTitle: String, contentImage: Image) -> some View{
            ...
            ScrollView {
                VStack(alignment: .center){
                    TabView(selection: $viewModel.selection){
                        makeHeader(headerTitle: "ひとこまから学びを始めよう", contentImage: Image("header1"))
                            .tag(0)
                        makeHeader(headerTitle: "あなたの知識が報酬に", contentImage: Image("header2"))
                            .tag(1)
                        makeHeader(headerTitle: "わからないことは何でも聞いてみよう", contentImage: Image("header3"))
                            .tag(2)
                    }
                    .frame(width: UIScreen.main.bounds.width-30,height:200)
                    .redacted(reason: app.isLoading ? .placeholder: .privacy)
                }
                .cornerRadius(15)
                .padding(.top,15)
                .onAppear{
