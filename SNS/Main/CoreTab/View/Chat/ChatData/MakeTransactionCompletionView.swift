    ...
    var body: some View {
        VStack {
            VStack(alignment: .leading){
                ...
                Divider()
                Text("取引終了メッセージ").bold().padding(.leading, 20)
                TextEditor(text: $lastMessage).frame(width: UIScreen.main.bounds.width-40, height: 150).background(Rectangle().stroke(.black, lineWidth: 1)).padding(.leading, 20)
            }
            Button {
                SetToFirestore().registerMessage(path:"Chat", chatRoomId: messageListData.chatRoomData?.chatroomId ?? "" ,messageText: messageListData.lastMessage, messageDate: messageListData.lastMessageDate, messageType: .finish) {
                    self.closeScreen = nil
                }
            } label: {
                Text("取引完了を申請する").foregroundColor(.white).bold().frame(width: UIScreen.main.bounds.width-40, height: 40).background(.black).cornerRadius(10).padding(.horizontal, 16).padding(.top, 10)
            }
            Button {
                self.closeScreen = nil
            } label: {
