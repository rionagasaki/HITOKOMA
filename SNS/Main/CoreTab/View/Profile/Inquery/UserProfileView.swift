...
struct UserBaseProfileView: View {
    ...
    var body: some View{
        VStack(alignment: .leading, spacing: .zero){
            ...
            HStack{
                WebImage(url: URL(string: userProfileImageURL)).resizable().frame(width: 70, height: 70).clipShape(Circle()).background(Circle().stroke(.white, lineWidth: 5)).padding(.leading, 16).padding(.top, -35)
                Spacer()
                Button {
                    print("aaa")
                } label: {
            ...
            Text(username).bold().padding(.leading, 16).padding(.top, 5)
            Text("20代/男性/千葉県").foregroundColor(.init(uiColor: .lightGray)).font(.caption).font(.system(size: 12)).padding(.leading, 16)
            HStack{
                Text("機密保持契約(NDA)").foregroundColor(.black).font(.caption)
...
struct UserSubBaseProfileView: View {
    var body: some View{
        ...
        Text("このユーザーのリクエスト").font(.system(size: 18)).bold().padding(.leading, 16)
        Divider().padding(.vertical, 10)
    }
}
